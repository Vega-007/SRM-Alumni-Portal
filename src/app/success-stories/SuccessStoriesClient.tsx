...
import { MapPin, Users, Quote, ChevronDown, ChevronUp, Globe } from "lucide-react";
import { ErrorBoundary } from "@/components/ErrorBoundary";
import { WorldMapPaths } from "@/components/Hero/WorldMapPaths";
import { ThreeDGlobe } from "@/components/Hero/ThreeDGlobe";

export default function SuccessStoriesPage() {
  const [activeHub, setActiveHub] = useState<string | null>(null);
  const [expandedStoryId, setExpandedStoryId] = useState<string | null>(null);
  const [viewMode, setViewMode] = useState<"classic" | "crisp" | "3d">("classic");
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-20">
          <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800/60 bg-white/60 dark:bg-slate-950/60 backdrop-blur-md p-6 md:p-8 relative overflow-hidden shadow-sm">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-800 mb-6 gap-2 overflow-x-auto">
              <button
                ...
                className={`pb-2.5 px-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-all cursor-pointer ${
                  viewMode === "classic"
                    ? "border-srm-yellow text-slate-900 dark:text-white"
              ...
              <button
                ...
                className={`pb-2.5 px-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-all cursor-pointer flex items-center gap-1.5 ${
                  viewMode === "3d"
                    ? "border-srm-yellow text-slate-900 dark:text-white"
                    : "border-transparent text-slate-500 hover:text-slate-900 dark:hover:text-white"
                }`}
            ...
            <div className="w-full h-[400px] md:h-[500px] relative border border-slate-200 dark:border-slate-900 rounded-2xl bg-slate-100/80 dark:bg-slate-950/40 overflow-hidden shadow-inner backdrop-blur-sm flex items-center justify-center">
              ...
              <ErrorBoundary>
                  ...
                  <>
                    ...
                    <svg viewBox="0 0 1000 500" className="absolute inset-0 w-full h-full pointer-events-none fill-none stroke-current" strokeWidth="0.5" strokeOpacity="0.4">
                      {viewMode === "classic" ? (
                        ...
                        <g className="fill-slate-200/40 dark:fill-slate-800/30 stroke-slate-350 dark:stroke-slate-700/50 transition-colors duration-500" strokeWidth="1.2">
                          <g transform="translate(-240, 0)">
                            <g transform="scale(1.275, 1.09) translate(-30.767, -241.591)">
                              <WorldMapPaths />
                            </g>
                          </g>
                              ...
                              <div className="border-t border-slate-200 dark:border-slate-900 mt-2 pt-2">
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {hub.topCompanies.map((c) => (
                                    <span key={c} className="text-[9px] bg-slate-200 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400 rounded px-1 py-0.5">
                                      {c}
                                    </span>
                                  ))}
                                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4 md:right-auto bg-slate-200/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-300 dark:border-slate-800 rounded-xl p-3 max-w-sm pointer-events-none z-30">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-3 flex flex-col items-center sm:items-start text-center sm:text-left gap-4">
                      <div className="h-28 w-28 rounded-2xl overflow-hidden bg-slate-200 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 shrink-0 relative">
                        <Image
                          src={story.imageUrl}
                          alt={story.name}
                          fill
